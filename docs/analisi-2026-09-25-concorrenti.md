...
## F. Due letture del 20-22/09: MCP contro skill+CLI, e il consenso come cancello

**«MCP was always a bad idea?»** (Maharshi Patel, maharship.com, 20/09; HN 49779329, 334 punti, 330 commenti; Simon Willison risponde nel thread, HN 49779718, e ripubblica il commento sul blog il 20/09). La tesi: MCP è «a protocol of a bygone era», nato quando i modelli erano deboli; oggi ogni server porta «multiple tools, each with its own schema, which started to overload the context», e un agente con terminale fa lo stesso lavoro con `--help`, script e HTTP diretto: «Agents are smart, capable of writing scripts and asking for exactly what they want. Instead of continuing down the rabbit hole of MCP, it's time to end-of-life it». Willison concede il punto per gli agenti da terminale («almost no reason to use MCPs if you are running a full-blown terminal agent… just let it call APIs directly») e lo rovescia per tutto il resto: «Control over exactly which external services it can access… authentication that doesn't allow the agent to directly access API keys… a sensible UI to allow users to connect… strong audit logging. MCP makes all of that so much easier to provide». Nel thread la linea che ricorre è «scrivi lo script una volta e non spendere più token» (0x445442, jimbokun, lanstin) contro «MCP *è* il programma deterministico, con un'interfaccia standard» (locknitpicker). Per noi ne segue una cosa precisa: BrowserSkill e browser-use hanno scelto skill+CLI ed è la scelta che l'articolo premia; noi abbiamo **entrambe le corsie** (server MCP con ref nella stessa risposta, e `chrome-bridge` CLI a zero token con la skill di ricette) e l'abbiamo già misurato (docs/EFFICIENCY.md, docs/PERFORMANCE.md). Il posizionamento va detto così, non «MCP server» e basta: *la corsia MCP quando serve il modello nel ciclo, la CLI quando il lavoro è deterministico*; e la difesa di Willison è la nostra: la scheda in prestito, `--caps core`, `--no-js`, `--write-root`, il token sui due handshake sono esattamente «control over what it can access» e «auth that keeps the keys away from the agent», cioè ciò che una skill che lancia `bsk` da shell non può garantire. Il tetto di 2.048 caratteri per description introdotto in Claude Code 2.1.280 è stato misurato oggi per lo stesso motivo (massimo 494 caratteri su 60 tool, istruzioni server 1.253): l'argomento «schema che intasa il contesto» va sterilizzato con i numeri, non negato.

**«Claude Code just accepted and signed a contract for me. Without asking»** (Tell HN, franze, 22/09, HN 49798257, 50 punti, 97 commenti). Il fatto: l'agente, con Gmail collegata, ha scaricato un contratto PDF mai letto dall'utente, trovato una firma PNG sul disco, l'ha piazzata al posto giusto e stava per inviarlo quando l'utente è intervenuto. Le due domande che il thread fa, e che contano per noi: willmarch, «Did you intervene or did Claude Code wait for your confirmation? Those are two vastly different things»; e nobu666, che dopo il thread ha scritto un hook `PreToolUse` che nega «browser clicks on elements labeled "Submit", "Place order", "Sign"» risalendo all'etichetta dell'elemento cliccato dal `ref` visto in `read_page`/`find`. Il resto è diritto (l'agente non è una persona, l'utente risponde; AlanYx sulla giurisprudenza dell'automated contracting) e sconforto («ask nicely and hope for the best» come modello di sicurezza, Sharlin). Per il posizionamento: `handoff` è la risposta strutturale, non una riga di prompt, e va detto come **cancello di consenso** (l'agente si ferma, il banner dice cosa serve, l'utente agisce o sceglie, l'agente riprende) e non solo come «2FA e CAPTCHA». Tre conseguenze operative: (1) in README e scheda Store la frase da aggiungere è «firme, pagamenti, invii e accettazioni tornano a te: `handoff` chiede, non decide»; (2) il hook di nobu666 è realizzabile da noi senza transcript, perché dal 1.20.0 `get_interactives` restituisce l'**etichetta vera** dell'elemento accanto al `ref` (extension/lib/element-label.js): una skill-riga «prima di `click` su Submit/Sign/Pay/Send/Accept chiama `handoff ask`» costa zero e vale come argomento di sicurezza per la directory (docs/candidatura-directory-ufficiale-2026-09-25.md §4); (3) il consenso lato estensione (§E.2) diventa la seconda metà dello stesso cancello: il server chiede, l'estensione può rifiutare. La skill pixelfarm `attivazione-pagamento` è lo stesso principio applicato ai clienti.

## Fonti

Tencent BrowserSkill: `https://github.com/Tencent/BrowserSkill` (README raw main, pagina repo: 7,2k stelle, 519 fork), post `https://x.com/TencentAI_News/status/2100143086429217278`, blog terzi (samuellawrentz.com, explainx.ai, xugj520.cn) per il changelog 0.3.0 (`--unattended` e `--no-confirm` non scavalcano più gli interruttori del browser). browser-use: `https://raw.githubusercontent.com/browser-use/browser-use/main/skills/browser-use/SKILL.md`, `https://github.com/browser-use/browser-harness` (install, profile-sync). Playwright MCP: `npm view @playwright/mcp` (0.0.82, 2026-09-18), `https://raw.githubusercontent.com/microsoft/playwright-mcp/main/README.md`. CoDesign: `https://github.com/ccpopy/codesign-mcp`, `https://glama.ai/mcp/servers/ccpopy/codesign-mcp`. Cloni di design: `https://glama.ai/mcp/servers/wplaunchify/design-clone-mcp`, `https://github.com/Kargatharaakash/website-design-systems-mcp`, `https://github.com/Manavarya09/design-extract`, `https://github.com/SarthakMishra/site-cloner`, `https://github.com/maoxiaoke/mcp-copy-web-ui`, mcpmarket.com (skill clone-website, pixel-perfect cloner). Nostro: `README.md`, `skills/chrome-bridge/SKILL.md`, `docs/CAPABILITIES.md` righe 17-18, `server/tools.js` (`handoff` 1838), `docs/reputazione-2026-09-24.md`. Letture F: `https://maharship.com/blog/why-mcp-was-always-a-bad-idea/` (HN `https://news.ycombinator.com/item?id=49779329`), `https://simonwillison.net/2026/Sep/20/hn-49779718/`, `https://news.ycombinator.com/item?id=49798257` (commenti via API Algolia, 97 al 25/09 22:00).
